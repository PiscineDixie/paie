...
def deploy(c):
    ...
    c.run("cd paie && env BUNDLE_DEPLOYMENT=1 BUNDLE_WITHOUT=development bundle install")
    c.run("cd paie && env RAILS_ENV=production bin/rails assets:precompile")
    c.run("cd paie && env RAILS_ENV=production bundle exec rake db:migrate")
