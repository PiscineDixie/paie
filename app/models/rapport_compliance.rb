...
class RapportCompliance
  ...
  def initialize
    ...
    Feuille.
      ...
      where('empl.naissance > ?', minDate.to_formatted_s(:db)).each do | f |
      f.heures.each do | hr |
        0.step(hr.duree-1, 60) do | minsFromStart |
          ...
          if ageMin.to_date < f.employe.naissance
            ...
            hr2 = Heure.find_by_sql(
               ...
               {:start => dateTime.to_formatted_s(:db), :fin => (dateTime+3600).to_formatted_s(:db), :ageMin => ageMin.to_formatted_s(:db)}])
            if !hr2 || hr2.empty?
